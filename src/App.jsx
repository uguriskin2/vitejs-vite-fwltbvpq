...
import { toTRUpper, getInitialQuestion, getCorrectAnswerText, getGivenAnswerText, shuffleArray } from './utils/helpers';
import { 
  IconTarget, IconLock, IconLogOut, IconUser, IconPlusCircle, 
  IconLink, IconTrash2, IconEdit, IconChevronLeft, IconChevronRight, 
  IconDownload, IconPrinter, IconBookOpen, IconClock, 
  IconCheckCircle, IconXCircle, IconSend, IconActivity, IconActivitySmall, 
  IconFileText, IconImage 
...
const App = () => {
  ...
  useEffect(() => {
    const forceUnlock = setTimeout(() => { setLoading(false); }, 2500);
  ...
  useEffect(() => {
      const handleAway = () => { if (view === 'exam' && !isAway.current) { isAway.current = true; setCheatWarnings(prev => prev + 1); } };
      const handleReturn = () => { if (view === 'exam' && isAway.current) { isAway.current = false; } };
      const onVisibilityChange = () => { if (document.hidden) handleAway(); else handleReturn(); };
  ...
  async function callGemini(prompt, systemInstruction = "", fileBase64 = null, mimeType = null) {
    ...
    const FIREBASE_KEY_CHECK = "AIzaSyDLcdpVEUHpaUP2IgMuYeszaHmmhODcHH8";
    if (currentKey === FIREBASE_KEY_CHECK) throw new Error("DİKKAT: Kutucuğa yapay zeka yerine FIREBASE şifrenizi girdiniz! Lütfen aistudio.google.com adresinden YENİ bir anahtar alıp kutucuğa yapıştırın.");
  ...
  const handleEditExam = (exam) => { setNewExam(exam); setCurrentQuestion(getInitialQuestion()); setEditingQIdx(null); setView('create'); };
  ...
  const processFile = (file) => {
      ...
      if (!allowedTypes.includes(file.type)) { showModal("Hata", "Lütfen sadece PDF veya Resim (PNG, JPG) dosyası yükleyin.", "error"); return; }
      if (file.size > 5 * 1024 * 1024) { showModal("Hata", "Dosya boyutu çok büyük. Lütfen en fazla 5MB olan bir dosya seçin.", "error"); return; }
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      instruction += `\nÖNEMLİ KURAL: Ekli dosyanın veya metnin orijinal dilini (İngilizce, Almanca, Türkçe vb.) tespit et ve soruları KESİNLİKLE METNİN ORİJİNAL DİLİNDE HAZIRLA. Asla çeviri yapma!`;
      instruction += `\nÇOK ÖNEMLİ KURAL (GÖRSEL YASAĞI): Okuduğun PDF veya metindeki resimlere, grafiklere, tablolara veya numaralandırılmış görsellere atıfta bulunan ("Yukarıdaki görsele göre", "Şekil 1'de...") sorular KESİNLİKLE ÜRETME! SADECE metinden, mantıktan veya genel kültürden çözülebilecek, görsele ihtiyaç duymayan sorular üret!`;

      if (uploadData && uploadType === 'application/pdf' && pageRange.trim()) { instruction += `\nÖNEMLİ DİKKAT: Ekli PDF dosyasının SADECE şu sayfalarındaki veya şu kısımlarındaki bilgileri kullanarak soru üret: "${pageRange}"`; }
      if (uploadData && uploadType.startsWith('image/')) { instruction += `\nÖNEMLİ DİKKAT: Lütfen ekli görseli (fotoğrafı) analiz ederek içindeki metin, grafik veya olaylara uygun sorular üret.`; }

      instruction += `\nYanıt SADECE VE SADECE geçerli bir JSON dizisi olmalıdır. Format Örneği: [{"text": "Soru metni", "topic": "Konu", "type": "multiple-choice", "options": ["A", "B", "C", "D"], "correct": 0, "correctText": "", "pairs": [{"left": "Terim", "right": "Açıklama"}]}]`;
      ...
      try {
        ...
        if (e.message === "API_KEY_ERROR") { showModal("Geçersiz Şifre", "Girdiğiniz API Anahtarı iptal edilmiş veya hatalı.", "error"); } 
        else if (e.message.includes("DEMO_MODE_TRIGGER")) {
             const demoQuestions = [{ text: "Türkiye'nin başkenti neresidir?", topic: "Coğrafya", type: "multiple-choice", options: ["İstanbul", "Ankara", "İzmir", "Antalya"], correct: 1, correctText: "", pairs: [{left: '', right: ''}] }];
             setNewExam(prev => ({ ...prev, questions: [...(prev.questions || []), ...demoQuestions] }));
             setAiText(''); setUploadData(null); setUploadName(''); setUploadType(''); setPageRange('');
             showModal("🚀 Örnek Sorular Yüklendi", "Sistem çalışıyor ancak Google API şifresi yetkisiz.", "success");
  ...
  const handleStudentStart = () => {
    ...
    const foundExam = exams.find(e => e.examCode && toTRUpper(e.examCode) === toTRUpper(studentExamCode));
    if (!foundExam) {
        showModal("Hatalı Kod", `"${studentExamCode}" koduna ait bir sınav bulunamadı.`, "error");
        return;
    }
    setCheatWarnings(0); isAway.current = false;
    const shuffledQuestions = shuffleArray(foundExam.questions || []);
    setActiveExam({...foundExam, questions: shuffledQuestions}); 
    setAnswers({}); setTimeLeft(foundExam.duration * 60); setView('exam');
  };
  ...
  const handleFinishExam = async (forceParam) => {
    ...
    (activeExam.questions || []).forEach((q, idx) => {
        ...
        let isMissing = false;
        if (ans === undefined || ans === null) isMissing = true;
        else if (q.type === 'short-answer' && String(ans).trim() === '') isMissing = true;
        else if (q.type === 'matching') {
            if (typeof ans !== 'object') isMissing = true;
            else { const hasAll = q.pairs.every(p => ans[p.left] && String(ans[p.left]).trim() !== ''); if (!hasAll) isMissing = true; }
        }
    ...
    const submitData = async () => {
        ...
        const questionDetails = (activeExam.questions || []).map((q, idx) => {
          ...
          if (q.type === 'short-answer') {
              ...
              if (!ansNorm) { earnedScore = 0; } 
              else if (correctNorm === ansNorm) { earnedScore = 1; } 
              else {
                  const correctWords = correctNorm.split(/\s+/).filter(w => w.length > 0);
                  const ansWords = ansNorm.split(/\s+/).filter(w => w.length > 0);
                  if (correctWords.length === 0) { earnedScore = 0; } 
                  else {
                      const ansWordsSet = new Set(ansWords); let matchCount = 0;
                      correctWords.forEach(cw => { if (ansWordsSet.has(cw)) matchCount++; });
          ...
          return { 
              topic: q.topic || 'Genel', isCorrect, earnedScore, questionText: q.text,
              givenAnswerText: getGivenAnswerText(q, ans), correctAnswerText: getCorrectAnswerText(q)
          };
        });
        ...
        const submissionData = {
            ...
            score: finalScorePercentage, correctCount: totalEarned, totalQuestions: activeExam.questions.length, 
            questionDetails, submittedAt: new Date().toISOString(), cheatWarnings: finalCheatCount 
        };
        ...
        try {
          ...
          setActiveExam(null); setAnswers({}); setStudentExamCode(''); setView('result'); 
        } catch (e) {
    ...
    if (!isForced && unanswered.length > 0) {
        showModal("Cevapsız Sorularınız Var!", `Şu soruları boş bıraktınız veya eksik cevapladınız:\n👉 Soru: ${unanswered.join(', ')}\n\nEksik sorularınız varken yine de sınavı bitirmek istiyor musunuz?`, "confirm", submitData);
    } else submitData();
  };

  const handleDeleteSubmission = (subId) => { showModal("Sonucu Sil", "Bu öğrencinin sınav sonucunu kalıcı olarak silmek istediğinize emin misiniz?", "confirm", async () => { try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'submissions', subId)); } catch (error) { setSubmissions(prev => prev.filter(s => s.id !== subId)); } }); };
  const handleDeleteAllSubmissions = (examId) => { showModal("Tümünü Sil", "Bu sınava ait TÜM öğrenci sonuçlarını kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz.", "confirm", async () => { try { const subsToDelete = submissions.filter(s => s.examId === examId); for (const sub of subsToDelete) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'submissions', sub.id)); } setSelectedSubs([]); } catch (error) { setSubmissions(prev => prev.filter(s => s.examId !== examId)); } }); };
  const toggleSubSelection = (subId) => { setSelectedSubs(prev => prev.includes(subId) ? prev.filter(id => id !== subId) : [...prev, subId]); };
  const toggleAllSubs = (currentExamSubs) => { if (selectedSubs.length === currentExamSubs.length) setSelectedSubs([]); else setSelectedSubs(currentExamSubs.map(s => s.id)); };
  const handleDeleteSelected = () => { showModal("Seçilenleri Sil", `${selectedSubs.length} adet öğrenci sonucunu kalıcı olarak silmek istediğinize emin misiniz?`, "confirm", async () => { try { for (const subId of selectedSubs) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'submissions', subId)); } setSelectedSubs([]); } catch (error) { setSubmissions(prev => prev.filter(s => !selectedSubs.includes(s.id))); setSelectedSubs([]); } }); };
  ...
  const handleExportExcel = () => {
    ...
    let tableHtml = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body><table border="1"><thead><tr><th style="background-color: #4f46e5; color: white;">Öğrenci Adı Soyadı</th><th style="background-color: #4f46e5; color: white;">Okul Numarası</th><th style="background-color: #4f46e5; color: white;">Sınıf / Şube</th><th style="background-color: #4f46e5; color: white;">Doğru Soru Sayısı</th><th style="background-color: #4f46e5; color: white;">Toplam Puanı</th><th style="background-color: #4f46e5; color: white;">Sınav Tarihi ve Saati</th><th style="background-color: #4f46e5; color: white;">Kural İhlali Sayısı</th><th style="background-color: #4f46e5; color: white;">Cihaz ID</th></tr></thead><tbody>`;
    examSubs.forEach(s => { tableHtml += `<tr><td>${s.studentName || '-'}</td><td>${s.studentNumber || '-'}</td><td>${s.studentClass || '-'}</td><td style="text-align: center;">${Number(s.correctCount).toFixed(1).replace('.0', '')}</td><td style="text-align: center; font-weight: bold;">${Number(s.score).toFixed(0)}</td><td>${s.submittedAt ? new Date(s.submittedAt).toLocaleString('tr-TR') : '-'}</td><td style="text-align: center;">${s.cheatWarnings || 0}</td><td style="color: #666;">${s.deviceId || '-'}</td></tr>`; });
    tableHtml += `</tbody></table></body></html>`;
    const blob = new Blob([tableHtml], { type: 'application/vnd.ms-excel;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a'); link.href = url; link.download = `Sinav_Sonuclari_${activeExam?.examCode || 'Liste'}.xls`;
  ...
  const getTopicAnalysis = (examId) => {
    ...
    examSubs.forEach(sub => {
      (sub.questionDetails || []).forEach(d => {
        ...
        stats[tName].total++; stats[tName].correct += (d.earnedScore !== undefined ? d.earnedScore : (d.isCorrect ? 1 : 0));
      });
    });
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      ...
      <nav className="bg-white border-b border-slate-200 p-4 sticky top-0 z-50 flex justify-between items-center shadow-sm print:hidden">
        <div className="flex items-center gap-2 font-black text-xl sm:text-2xl text-indigo-600 cursor-pointer" onClick={() => { setView('landing'); setExamResult(null); }}>
          ...
          <div className="flex flex-col"><span>SINAV<span className="text-slate-800">UI</span></span><span className="text-[9px] text-slate-400 font-bold tracking-widest lowercase -mt-1 hidden sm:block">uguriskin@gmail.com</span></div>
        </div>
      ...
      <main className="max-w-6xl w-full mx-auto p-4 md:p-8 flex-1">
        {view === 'landing' && (
          <div className="text-center py-10 sm:py-20 animate-in fade-in zoom-in print:hidden px-4">
            ...
            <p className="text-sm sm:text-lg md:text-xl text-slate-400 mb-10 sm:mb-14 max-w-2xl mx-auto font-bold leading-relaxed px-4">Sınavda Başarılar...<br/>Uğur ISKIN</p>
   ...
   <input className="w-full p-4 sm:p-5 bg-indigo-50 rounded-xl sm:rounded-[2rem] font-black text-sm sm:text-base outline-none shadow-inner border-2 border-indigo-200 focus:border-indigo-500 transition-colors uppercase tracking-widest text-indigo-700 placeholder-indigo-300" placeholder="GİRİŞ KODU (Örn: FEN6)" value={newExam.examCode} onChange={e => setNewExam({...newExam, examCode: toTRUpper(e.target.value)})} />
                  ...
                  <div className="bg-indigo-900 rounded-2xl sm:rounded-[3rem] p-5 sm:p-10 text-white shadow-xl sm:shadow-2xl mb-8 sm:mb-12">
                     ...
                     <div className="mb-6 sm:mb-8 p-4 sm:p-5 bg-black/20 rounded-xl sm:rounded-2xl border border-white/10">
                        ...
                        <input type="password" placeholder="AIzaSy ile başlayan anahtarınızı buraya yapıştırın..." value={geminiKey} onChange={e => { setGeminiKey(e.target.value); localStorage.setItem('gemini_api_key', e.target.value); }} className="w-full bg-white/10 border border-white/20 rounded-lg sm:rounded-xl p-2.5 sm:p-3 outline-none text-xs sm:text-sm text-white focus:border-indigo-400 transition-colors placeholder:text-white/30" />
                     ...
                     <div className="mb-4 sm:mb-6 space-y-3 sm:space-y-4">
                        <div className="bg-white/5 p-4 sm:p-5 rounded-xl sm:rounded-3xl border border-white/10">
                            ...
                            {!uploadData ? (
                                <label className="flex flex-col items-center justify-center w-full h-24 sm:h-28 border-2 border-dashed border-indigo-300/30 rounded-xl sm:rounded-2xl cursor-pointer hover:bg-white/10 transition-all text-center px-4">
                                    <div className="flex gap-2 text-indigo-200 mb-1 sm:mb-2"><IconFileText size={20} /><IconImage size={20} /></div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-4 opacity-30"><div className="h-px bg-white flex-1"></div><span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest">VEYA MANUEL METİN GİRİN</span><div className="h-px bg-white flex-1"></div></div>
                  ...
                  <div className={"p-6 sm:p-10 rounded-2xl sm:rounded-[3rem] space-y-6 sm:space-y-8 border shadow-inner transition-colors " + (editingQIdx !== null ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200')}>
                    <h3 className={"font-black text-lg sm:text-xl uppercase tracking-widest " + (editingQIdx !== null ? 'text-indigo-900' : 'text-slate-800')}>{editingQIdx !== null ? `${editingQIdx + 1}. Soruyu Düzenle` : 'Manuel Soru Ekle'}</h3>
                    ...
                    {editingQIdx !== null && ( <button type="button" onClick={() => { setCurrentQuestion(getInitialQuestion()); setEditingQIdx(null); }} className="w-full text-slate-500 py-3 rounded-xl font-bold text-xs uppercase hover:bg-slate-200 transition-colors">Düzenlemeyi İptal Et</button> )}
                  </div>
               </div>
            </div>
            <div className="md:col-span-4 space-y-6">
              <div className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-[3.5rem] shadow-sm border border-slate-100 sticky top-24">
                 <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6 sm:mb-8">
                     <div><h3 className="font-black text-xl sm:text-2xl uppercase tracking-tighter text-slate-900">Taslak</h3><p className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase mt-1">{newExam.questions.length} Soru Eklendi</p></div>
                      ...
                      <div key={i} className={"p-4 sm:p-5 rounded-xl sm:rounded-[2rem] border relative group transition-all " + (editingQIdx === i ? 'bg-indigo-50 border-indigo-300 shadow-md' : 'bg-slate-50 border-slate-200 hover:border-indigo-300 hover:shadow-sm')}>
                          <div className="flex justify-between items-start mb-2 sm:mb-3">
                              ...
                              <div className="flex gap-1">
                                  ...
                                  <button type="button" onClick={()=>{ if(editingQIdx === i) { setCurrentQuestion(getInitialQuestion()); setEditingQIdx(null); } else if (editingQIdx > i) { setEditingQIdx(prev => prev - 1); } setNewExam({...newExam, questions:newExam.questions.filter((_,idx)=>idx!==i)}) }} className="text-slate-300 hover:text-red-500 bg-white p-1.5 rounded-lg shadow-sm border border-slate-100 transition-colors"><IconTrash2 size={14}/></button>
                              </div>
                          </div>
                 ...
                 <div className="md:col-span-8 bg-white rounded-2xl sm:rounded-[4rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
                    <div className="p-5 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                        ...
                        <div className="flex items-center gap-2 sm:gap-4">
                            ...
                            {selectedSubs.length > 0 && ( <button type="button" onClick={handleDeleteSelected} className="text-[9px] sm:text-[10px] font-black bg-red-500 text-white hover:bg-red-600 px-3 sm:px-4 py-1 sm:py-1.5 rounded-md sm:rounded-full uppercase tracking-widest transition-colors flex items-center gap-1 print:hidden shadow-sm"><IconTrash2 size={12}/> Seçilenleri Sil ({selectedSubs.length})</button> )}
                            {submissions.filter(s=>s.examId===activeExam.id).length > 0 && selectedSubs.length === 0 && ( <button type="button" onClick={() => handleDeleteAllSubmissions(activeExam.id)} className="text-[9px] sm:text-[10px] font-black bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 px-3 sm:px-4 py-1 sm:py-1.5 rounded-md sm:rounded-full uppercase tracking-widest border border-red-100 transition-colors flex items-center gap-1 print:hidden"><IconTrash2 size={12}/> Tümünü Sil</button> )}
                        </div>
                    </div>
                            ...
                            <thead className="bg-slate-50 border-b uppercase text-[8px] sm:text-[10px] font-black text-slate-400 tracking-widest">
                                <tr>
                                    <th className="p-4 sm:p-6 text-center font-bold text-slate-300 print:hidden w-16">
                                        <div className="flex flex-col items-center justify-center gap-1"><span>Seç</span><input type="checkbox" checked={selectedSubs.length === submissions.filter(s=>s.examId===activeExam.id).length && submissions.filter(s=>s.examId===activeExam.id).length > 0} onChange={() => toggleAllSubs(submissions.filter(s=>s.examId===activeExam.id))} className="w-4 h-4 cursor-pointer accent-indigo-600" /></div>
                                    </th>
                                    ...
                                    <tr key={i} className={"transition-colors " + (selectedSubs.includes(sub.id) ? 'bg-indigo-50/50' : 'hover:bg-indigo-50/30')}>
                                        <td className="p-4 sm:p-6 text-center print:hidden w-16">
                                            <div className="flex flex-col items-center justify-center gap-3">
                                                ...
                                                <button type="button" onClick={() => handleDeleteSubmission(sub.id)} className="p-1.5 bg-white border border-slate-100 rounded-lg text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm" title="Bu öğrenciyi sil"><IconTrash2 size={14}/></button>
                                            </div>
                                        </td>
                              ...
                              <div key={i} className="bg-white border-2 border-slate-100 p-6 sm:p-10 rounded-2xl sm:rounded-[3rem] shadow-sm break-inside-avoid">
                                  <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b-2 border-slate-50 pb-4 sm:pb-6 mb-6 sm:mb-8 gap-4 sm:gap-0">
                                      ...
                                      <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center justify-between gap-4">
                                          ...
                                          <button type="button" onClick={() => handleDeleteSubmission(sub.id)} className="p-1.5 sm:p-2 bg-white border border-slate-200 rounded-md text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all print:hidden" title="Bu öğrencinin sonucunu sil"><IconTrash2 size={14}/></button>
                                      </div>
                                  </div>
             ...
             <div className="bg-white p-8 sm:p-12 md:p-16 rounded-3xl sm:rounded-[4rem] shadow-xl sm:shadow-2xl border border-slate-100 text-center relative overflow-hidden">
                ...
                <div className="space-y-3 sm:space-y-4 mb-8 sm:mb-10 relative z-10">
                    ...
                    <div className="flex gap-3 sm:gap-4">
                        ...
                        <input className="w-full p-4 sm:p-6 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-[2rem] text-center font-black text-base sm:text-xl shadow-inner outline-none focus:ring-2 sm:focus:ring-4 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all uppercase tracking-widest placeholder:text-slate-300" placeholder="SINIF/ŞUBE" value={studentClass} onChange={e => setStudentClass(toTRUpper(e.target.value))} />
                    </div>
                    <div className="pt-2 sm:pt-4">
                        <input className="w-full p-4 sm:p-6 bg-indigo-50 border-2 border-indigo-200 rounded-xl sm:rounded-[2rem] text-center font-black text-xl sm:text-3xl shadow-inner outline-none focus:border-indigo-500 focus:ring-2 sm:focus:ring-4 focus:ring-indigo-500/20 transition-all uppercase tracking-[0.2em] text-indigo-700 placeholder-indigo-300" placeholder="SINAV KODU" value={studentExamCode} onChange={e => setStudentExamCode(toTRUpper(e.target.value))} />
                    </div>
                </div>

                <button type="button" onClick={handleStudentStart} className="w-full bg-indigo-600 text-white py-5 sm:py-6 rounded-xl sm:rounded-[3rem] font-black text-xl sm:text-2xl hover:scale-105 active:scale-95 transition-all shadow-xl sm:shadow-2xl flex items-center justify-center gap-3 relative z-10">BAŞLA <IconChevronRight size={28} /></button>
             </div>
          </div>
        )}
        ...
        {view === 'result' && examResult && (
          <div className="max-w-3xl mx-auto py-6 sm:py-12 animate-in slide-in-from-bottom-8 no-print px-4">
             <div className="bg-white p-8 sm:p-16 rounded-3xl sm:rounded-[4rem] shadow-xl sm:shadow-2xl border border-slate-100 text-center relative overflow-hidden">
                ...
                <div className="flex justify-center mb-12"><div className={"w-40 h-40 sm:w-48 sm:h-48 rounded-[3rem] flex flex-col items-center justify-center shadow-inner border-8 " + (examResult.score >= 50 ? 'bg-green-50 border-green-100 text-green-600' : 'bg-red-50 border-red-100 text-red-600')}><span className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">PUAN</span><span className="text-5xl sm:text-6xl font-black tracking-tighter">{examResult.score.toFixed(0)}</span></div></div>
        ...
        {view === 'exam' && activeExam && (
          <div className="max-w-4xl mx-auto pb-20 sm:pb-32 animate-in slide-in-from-right no-print select-none" onCopy={(e) => { e.preventDefault(); showModal("Yasak", "Sınavda kopyalama işlemi kural ihlalidir.", "error"); }} onPaste={(e) => { e.preventDefault(); showModal("Yasak", "Sınavda yapıştırma işlemi kural ihlalidir.", "error"); }} onContextMenu={(e) => e.preventDefault()} >
